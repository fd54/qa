...
def train_entry():
    from model.models import QANet

    with open(config.word_emb_file, "r") as fh:
        word_mat = np.array(json.load(fh), dtype=np.float32)
    with open(config.char_emb_file, "r") as fh:
        char_mat = np.array(json.load(fh), dtype=np.float32)
    with open(config.train_eval_file, "r",encoding='utf8') as fh:
        train_eval_file = json.load(fh)
    with open(config.dev_eval_file, "r",encoding='utf8') as fh:
        dev_eval_file = json.load(fh)

    print("Building model...")

    train_dataset = DuReaderDataset(config.train_record_file, config.num_steps, config.batch_size)
    dev_dataset = DuReaderDataset(config.dev_record_file, config.test_num_batches, config.batch_size)
...
def test_entry():
    with open(config.dev_eval_file, "r") as fh:
        dev_eval_file = json.load(fh)
    dev_dataset = DuReaderDataset(config.dev_record_file, -1, config.batch_size)
    fn = os.path.join('..', config.save_dir, "model.pt")
    model = torch.load(fn)
